...
while let sample=output.copyNextSampleBuffer() {
 ...
 var sum=0.0,dark=0,count=0,brightest=0.0,difference=0.0
 var values:[Double]=[]
 for y in Swift.stride(from:height/10,to:height*9/10,by:max(1,height/100)) {
  for x in Swift.stride(from:width/10,to:width*9/10,by:max(1,width/100)) {
   ...
   values.append(luma);if reference.count>count{difference+=abs(luma-reference[count])}
   sum+=luma;brightest=max(brightest,luma);if luma<10{dark+=1};count+=1
  }
 }
 if reference.isEmpty{reference=values}
 rows.append(["difference_from_first":difference/Double(count),"seconds":CMTimeGetSeconds(CMSampleBufferGetPresentationTimeStamp(sample)),"mean":sum/Double(count),"dark_fraction":Double(dark)/Double(count),"max":brightest])
